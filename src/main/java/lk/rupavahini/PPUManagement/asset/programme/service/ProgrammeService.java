...
public class ProgrammeService implements AbstractService <Programme, Integer> {
    ...
    public List<ProgrammeModel> getallProgramme() {
        List<Programme> all = programmeDao.findAll();
        List<ProgrammeModel> programmeList=new ArrayList<>();
        for (Programme programme:all){
            programmeList.add(new ProgrammeModel(programme.getId(),programme.getProgrammeName()));
        }
        System.out.println(programmeList.toArray());
