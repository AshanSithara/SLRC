...
=======
    ...
    @Query(value = "select new lk.rupavahini.PPUManagement.asset.commonAsset.model.EventHourReportModel(e.id,e.title,TIME_TO_SEC(TIMESTAMPDIFF(hour, e.start, e.finish)) as hourcount,e.studio.studioName as studio_name ) from Event e where date(e.start)=date(:searchdate) ",nativeQuery = false )
    public List<EventHourReportModel> reporteventhour(@Param("searchdate") String date);

    @Query(value = "select new lk.rupavahini.PPUManagement.asset.commonAsset.model.EventHourReportModel(sum(TIME_TO_SEC(TIMESTAMPDIFF(hour, e.start, e.finish))) as hourcount, date(e.start) as month_date ) from Event e where month(e.start)=month(:searchdate) group by date(e.start)  ",nativeQuery = false )
    public List<EventHourReportModel> returndaybyhourreport(@Param("searchdate") String date);
//
//    @Query("select b from Event b where b.start between ?1 and ?2 and b.end between ?1 and ?2")
//    List<Event> findByDatesBetween(Date start, Date end);
